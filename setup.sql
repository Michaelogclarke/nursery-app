-- setup.sql — full database setup + seed data
-- Creates all tables and loads 13 staff + 45 children.
-- Safe to re-run: uses IF NOT EXISTS for tables, TRUNCATE for seed data.
--
-- Usage:
--   psql "<connection_string>" -f setup.sql
--   npm run setup
...
CREATE TABLE IF NOT EXISTS rooms (
  ...
  age_group    TEXT  -- 'under_2s' | 'two_to_three' | 'three_plus'
);
...
CREATE TABLE IF NOT EXISTS staff (
  ...
  access_ni_status     TEXT NOT NULL DEFAULT 'pending', -- 'pending' | 'clear' | 'expired'
  is_active            BOOLEAN NOT NULL DEFAULT TRUE,
  created_at           TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
-- Clear existing seed data (preserves table structure)
TRUNCATE TABLE
  rota_entries,
  attendance,
  authorised_pickups,
...
VALUES
  ...
  ('Rachel',  'Adams',      '1993-08-30', '07700 900005', 'rachel.adams@nursery.test',     'Nursery Nurse',        'Level 3 Childcare and Education',                  'ANI-100005', '2023-02-14', '2026-05-13', 'clear',   true),
...
VALUES
  ...
  ('Sophie',   'Cooper',     '2021-02-11', (SELECT id FROM rooms WHERE name = 'Pre-School'), NULL,         'Speech therapy Thursdays', true),
...
SELECT
  c.id,
  'Grandparent ' || c.last_name,
  'Grandparent',
  '028 9' || LPAD((ROW_NUMBER() OVER (ORDER BY c.id))::text, 7, '0'),
  2
FROM children c
WHERE c.id % 2 = 0;
